...
const $resourceType = dom.querySelectorAll('[name="resourceType"]');
...
const $uiBlockImage = dom.querySelector('.kaleflower-field-image-editor__ui-image-resource');
const $uiBlockWeb = dom.querySelector('.kaleflower-field-image-editor__ui-web-resource');
const $uiBlockNone = dom.querySelector('.kaleflower-field-image-editor__ui-no-image');

if(!dom.querySelector('[name="resourceType"]:checked')){
	dom.querySelector('[name="resourceType"][value=""]').checked = true;
}
...
$resourceType.forEach(function(element) {
	element.addEventListener('change', function(event) {
		updateResourceTypeTab(this.value);
	});
	if (element.checked) {
		updateResourceTypeTab(element.value);
	}
});
 ...
 * リソースタイプタブのレイアウト更新
 */
function updateResourceTypeTab(value){
	$uiBlockImage.style.display = 'none';
	$uiBlockWeb.style.display = 'none';
	$uiBlockNone.style.display = 'none';
	if (value == 'web') {
		$uiBlockWeb.style.display = 'block';
	} else if(value == 'none') {
		$uiBlockNone.style.display = 'block';
	} else {
		$uiBlockImage.style.display = 'block';
	}
}
